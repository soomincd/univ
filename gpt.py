...
if uploaded_files and (st.session_state.current_files != [f.name for f in uploaded_files]):
    ...
    if len(uploaded_files) > 10:
        ...
        if failed_files and success_files:
            st.error(f"처리 실패한 파일: {', '.join(name for name, _ in failed_files)}")
            st.success(f"처리 완료된 파일: {', '.join(success_files)}")
        elif failed_files:
            st.error(f"모든 파일 처리 실패: {', '.join(name for name, _ in failed_files)}")
        elif success_files:
